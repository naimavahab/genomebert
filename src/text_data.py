...
def build_text_dataloader(
    ...
    '''
    loaded_dataset = load_from_disk(cfg.dataset.get('local', None))
    loaded_dataset = loaded_dataset.remove_columns('text')
    dataset = loaded_dataset.with_format("torch")
    dataset = dataset.train_test_split(test_size=0.05, seed=42)
    split=cfg.dataset.get('split', None)
    print('Split:',split)
    if split=='val':
      dataset = dataset['test']#.select(range(10000))
    else:
    ...
    sampler = composer.utils.dist.get_sampler(dataset)
